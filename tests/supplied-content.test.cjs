...
test('doctor profile preserves the supplied cardiology facts', () => {
  const data = fs.readFileSync(path.join(root, 'doctors-data.js'), 'utf8');
  for (const value of ['2005', '2006', '2013', '2017', '20 лет', 'С понедельника по четверг, по предварительной записи', 'Ультразвуковая диагностика', 'Терапия', 'Функциональная диагностика', 'Кардиология', 'Ишемическая болезнь сердца', 'Сосудистые патологии']) {
